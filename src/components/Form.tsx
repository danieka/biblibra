...
const inputField = (type: 'number' | 'text', fieldName: string, value: string, readonly: boolean, handler: (e: Event) => void) => 
    (
...
export const form = (fields: Field[]) => defineComponent(
{    
    name: "GraphForm",
    emits: ['input'],
    props: {
    ...
    setup(props) {
        ...
        return () => (
            <div>
                {fields.map(field => inputFieldForType(field.type)(field.name, props.value[field.name], props.readonly, partial(handler, field.name)))}
            </div>
        )
    }
})